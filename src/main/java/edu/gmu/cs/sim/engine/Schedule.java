...
package edu.gmu.cs.sim.engine;

import java.io.Serializable;

import edu.gmu.cs.ec.util.MersenneTwisterFast;
import edu.gmu.cs.sim.util.Bag;
import edu.gmu.cs.sim.util.Heap;
import edu.gmu.cs.sim.util.LocationLog;
...
public class Schedule implements java.io.Serializable {
    ...
    public String getTimestamp(double time, final String beforeSimulationString, final String afterSimulationString) {
        ...
        if (_time >= AFTER_SIMULATION) {
            return afterSimulationString;
        }
        if (_time == (long) _time) {
            return Long.toString((long) _time);
        }
    ...
    public synchronized boolean step(final SimState state) {
        ...
        synchronized (lock) {
            if (time == AFTER_SIMULATION || queue.isEmpty()) {
                ...
                inStep = false;
                return false;
            }  // bump the time for the queue.isEmpty() bit
            ...
            while (true) {
                ...
                if (substeps.numObjs > 1) {
                    if (shuffling) {
                        substeps.shuffle(random);  // no need to flip -- we're randomizing
                    } else {
                        substeps.reverse();  // they came out in reverse order; we need to flip 'em
                    }
                }

                // dump
                if (topSubstep < substeps.numObjs) {
                    topSubstep = substeps.numObjs;  // remember index of largest substep since we're violating clear()
                }
                ...
                if (currentKey == null || currentKey.time != time) {
                    break;  // looks like no more substeps at this timestamp
                }
            }
        }
        ...
        public int compareTo(Object obj) {
            ...
            if (time == time2)  // the most common situation
            {
                ...
                if (ordering == ordering2) {
                    return 0;  // the most common situation
                }
                if (ordering < ordering2) {
                    return -1;
                }
...
class IterativeRepeat implements Steppable, Stoppable {
    ...
    public String toString() {
        return "Schedule.IterativeRepeat[" + step + "]";
    }
}
