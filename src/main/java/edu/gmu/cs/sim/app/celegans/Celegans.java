...
public class Celegans extends SimState {
    ...
    public void start() {
        super.start();
        if (database == null) {
            database = new Cells();  // only load at start() time, and only ONCE
        }
        ...
        schedule.scheduleOnce(1000, -1, new Steppable() {
            public void step(SimState state) {
                state.kill();
            }
        });   // kill the simulation at around 600, before the cells all die at 1000
    }
