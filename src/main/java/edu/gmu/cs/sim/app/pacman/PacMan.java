...
package edu.gmu.cs.sim.app.pacman;
...
import edu.gmu.cs.sim.util.TableLoader;
...
public class PacMan extends SimState {
    ...
    public void start() {
        ...
        try {
            maze.setTo(TableLoader.loadPNMFile(PacMan.class.getResourceAsStream("images/maze0.pbm")));
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void resetGame() {
        ...
        try {
            maze.setTo(TableLoader.loadPNMFile(PacMan.class.getResourceAsStream("images/maze" + (level - 1) % MAX_MAZES + ".pbm")));
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void resetAgents() {
        ...
        // add the Pacs
        if (pacs.length > 1) {
            pacs[1] = new Pac(this, 1);  // schedule pac 1 first so he appears on the bottom initially
        }
