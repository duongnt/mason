...
public class Cell extends Object implements Steppable {
    ...
    public String getExpressionPattern() {
        return expressionPatterns[expressionPattern];
    }
        ...
        for (int x = 0; x < location_size; x++) {
            for (int y = x + 1; y < location_size; y++) {
                ...
                // swap
                {
                    double swap;
                    swap = location_x[y];
                    location_x[y] = location_x[x];
                    location_x[x] = swap;
                    swap = location_y[y];
                    location_y[y] = location_y[x];
                    location_y[x] = swap;
                    swap = location_z[y];
                    location_z[y] = location_z[x];
                    location_z[x] = swap;
                    swap = location_t[y];
                    location_t[y] = location_t[x];
                    location_t[x] = swap;
    ...
    public void postProcessBirthday(boolean root) {
        if (root && time_born == -1) {
            ...
            birthday = (int) time_born;
        } else if (parent.embryo_division_time != -1) {
            birthday = (int) parent.embryo_division_time;
        } else if (parent.birthday >= post_embryonic_birthday) {
            birthday = parent.birthday + 1;
        } else {
            birthday = post_embryonic_birthday;
        }

        /* Perform on daughters */
        for (int z = 0; z < num_children; z++) {
            daughters[z].postProcessBirthday(false);
        }
    ...
    public void modifyLocations(double x, double y, double z) {
        ...
        for (int j = 0; j < num_children; j++) {
            daughters[j].modifyLocations(x, y, z);
        }
    }
    ...
    public void postProcessDeathDay(boolean root) {
        ...
        */
        ...
        if (num_children != 0) {
            death_day = daughters[0].birthday;
        }
        // else if (fatedToDie) death_day=birthday+1;
        else {
            death_day = maximum_death_day;
        }
        /* Perform on daughters */
        for (int z = 0; z < num_children; z++) {
            daughters[z].postProcessDeathDay(false);
        }
    }
    ...
    public void postProcessLocation(boolean root) {
        ...
        } else                    // condition 4
        {
            ...
            if (lineage_name.equals("")) {
                ...
                if (sc == 'd') {
                    split_radius_distance[1] = parent.split_radius_distance[1] / 2;
                    birth_location_y -= split_radius_distance[1];
                } else if (sc == 'v') {
                    split_radius_distance[1] = parent.split_radius_distance[1] / 2;
                    birth_location_y += split_radius_distance[1];
                } else if (sc == 'a') {
                    split_radius_distance[0] = parent.split_radius_distance[0] / 2;
                    birth_location_x -= split_radius_distance[0];
                } else if (sc == 'p') {
                    split_radius_distance[0] = parent.split_radius_distance[0] / 2;
                    birth_location_x += split_radius_distance[0];
                } else if (sc == 'l') {
                    split_radius_distance[2] = parent.split_radius_distance[2] / 2;
                    birth_location_z -= split_radius_distance[2];
                } else if (sc == 'r') {
                    split_radius_distance[2] = parent.split_radius_distance[2] / 2;
                    birth_location_z += split_radius_distance[2];
                } else {
                    System.out.println("Whoa! Lineage name with no split characteristics: "
                        + lineage_name + " (" + official_name + ")");
                }
            }
/*            else if (parent==null)
...
*/
        ...
        /* Perform on daughters */
        for (int z = 0; z < num_children; z++) {
            daughters[z].postProcessLocation(false);
        }
    }
    ...
    public void descendentsOf(Cell cell, HashMap add_here) {
        ...
        for (int z = 0; z < num_children; z++) {
            daughters[z].descendentsOf(cell, add_here);
        }
    }
