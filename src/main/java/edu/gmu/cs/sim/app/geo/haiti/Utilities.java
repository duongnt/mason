...
public class Utilities {
    ...
    static public void gridReplace(IntGrid2D grid, int from, int to) {
        for (int i = 0; i < grid.getWidth(); i++) {
            for (int j = 0; j < grid.getHeight(); j++) {
                if (grid.get(i, j) == from) {
                    grid.set(i, j, to);
                }
            }
        }
    ...
    public IntGrid2D constructGradient(Center c, IntGrid2D roads, ObjectGrid2D locations,
        ...
        for (Object o : neighbors) {
            ...
            if (l == center) {
                continue; // don't recheck the center
            }
            ...
            for (Object p : neighborsNeighbors) {
                ...
                if (n == l) {
                    continue; // don't recheck this tile!
                }
    ...
    int roadCost(Location l, IntGrid2D roads) {
        if (roads.get(l.x, l.y) > 0) {
            return (int) Agent.ENERGY_TO_WALK_PAVED;
        }
