...
package edu.gmu.cs.sim.app.asteroids;
        ...
        if (sizes.length > 0) {
            ...
            for (int i = 0; i < sizes.length; i++) {
                ...
                if (i == sizes.length - 1) {
                    forceX = -sumForceX;
                    forceY = -sumForceY;
                }  // last one ought to balance out the others.  It's best if it's the biggest one, hence why we list smaller asteroids first
                else {
                    sumForceX += forceX;
                    sumForceY += forceY;
                }
