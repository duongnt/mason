...
import edu.gmu.cs.sim.field.grid.AbstractGrid2D;
import edu.gmu.cs.sim.field.grid.AbstractGrid3D;
import edu.gmu.cs.sim.field.grid.DoubleGrid2D;
import edu.gmu.cs.sim.field.grid.DoubleGrid3D;
import edu.gmu.cs.sim.field.grid.IntGrid2D;
import edu.gmu.cs.sim.field.grid.IntGrid3D;
import edu.gmu.cs.sim.portrayal.LocationWrapper;
import edu.gmu.cs.sim.portrayal.Portrayal;
...
import edu.gmu.cs.sim.portrayal3d.simple.ValuePortrayal3D;
...
public class ValueGridPortrayal3D extends FieldPortrayal3D {
    ...
    public void setField(Object field) {
        if (field instanceof IntGrid3D || field instanceof DoubleGrid3D ||
            field instanceof IntGrid2D || field instanceof DoubleGrid2D) {
    ...
    public double newValue(int x, int y, int z, double value) {
        if (field instanceof IntGrid2D || field instanceof IntGrid3D) {
            value = (int) value;
        }

        if (map.validLevel(value)) {
            return value;
        }
