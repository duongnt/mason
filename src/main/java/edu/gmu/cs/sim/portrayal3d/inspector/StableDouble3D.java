...
package edu.gmu.cs.sim.portrayal3d.inspector;

import edu.gmu.cs.sim.field.SparseField;
...
public class StableDouble3D implements StableLocation {
    ...
    void update() {
        ...
        if (pos == null) {
            exists = false;
        }  // purposely don't update x and y and z so they stay the same
        else {
            x = pos.x;
            y = pos.y;
            z = pos.z;
    ...
    public void setX(double val) {
        ...
        if (field instanceof Continuous2D) {
            ((Continuous2D) field).setObjectLocation(object, new Double2D(val, getY()));
            z = 0;
        } else {
            ((Continuous3D) field).setObjectLocation(object, new Double3D(val, getY(), getZ()));
        }
        x = val;
        exists = true;
    }
    ...
    public void setY(double val) {
        ...
        if (field instanceof Continuous2D) {
            ((Continuous2D) field).setObjectLocation(object, new Double2D(getX(), val));
            z = 0;
        } else {
            ((Continuous3D) field).setObjectLocation(object, new Double3D(getX(), val, getZ()));
        }
        y = val;
        exists = true;
    }
    ...
    public void setZ(double val) {
        ...
        if (field instanceof Continuous2D) {
            z = 0;
            return;
        }  // won't set anything anyway
        else {
            ((Continuous3D) field).setObjectLocation(object, new Double3D(getX(), getY(), val));
        }
        z = val;
        exists = true;
    }
