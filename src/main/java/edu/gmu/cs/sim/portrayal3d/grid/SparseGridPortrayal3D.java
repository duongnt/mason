...
public class SparseGridPortrayal3D extends SparseFieldPortrayal3D {
    ...
    public void setField(Object field) {
        if (field instanceof SparseGrid3D || field instanceof SparseGrid2D) {
            super.setField(field);
        } else {
            throw new RuntimeException("Invalid field for SparseGridPortrayal3D: " + field);
        }
    }
    ...
    public LocationWrapper completedWrapper(LocationWrapper w, PickIntersection pi, PickResult pr) {
        ...
        if (field instanceof SparseGrid2D) {
            d = new StableInt2D((SparseGrid2D) field, w.getObject());
        } else {
            d = new StableInt3D((SparseGrid3D) field, w.getObject());
        }
