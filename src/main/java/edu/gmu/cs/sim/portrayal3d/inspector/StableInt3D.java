...
public class StableInt3D implements StableLocation {
    ...
    public void setX(int val) {
        ...
        if (field instanceof SparseGrid2D) {
            ((SparseGrid2D) field).setObjectLocation(object, new Int2D(val, getY()));
            z = 0;
        } else {
            ((SparseGrid3D) field).setObjectLocation(object, new Int3D(val, getY(), getZ()));
        }
        x = val;
        exists = true;
    }
    ...
    public void setY(int val) {
        ...
        if (field instanceof SparseGrid2D) {
            ((SparseGrid2D) field).setObjectLocation(object, new Int2D(getX(), val));
            z = 0;
        } else {
            ((SparseGrid3D) field).setObjectLocation(object, new Int3D(getX(), val, getZ()));
        }
        y = val;
        exists = true;
    }
    ...
    public void setZ(int val) {
        ...
        if (field instanceof SparseGrid2D) {
            ...
            ((SparseGrid3D) field).setObjectLocation(object, new Int3D(getX(), getY(), val));
        }
        z = val;
        exists = true;
    }
