...
*/
    ...
    boolean shouldBuffer(Graphics2D graphics) {
        ...
        if (buffering == USE_BUFFER) {
            return true;
        } else if (buffering == DONT_USE_BUFFER) {
            return false;
        } else if (Display2D.isMacOSX) {
            return (graphics.getDeviceConfiguration().
                getDevice().getType() != GraphicsDevice.TYPE_IMAGE_BUFFER);
        } else if (Display2D.isWindows) {
    ...
    protected void hitOrDraw(Graphics2D graphics, DrawInfo2D info, Bag putInHere) {
        ...
        if (graphics != null && shouldBuffer(graphics)) {
            ...
            if (buffer == null || buffer.getWidth() != maxX || buffer.getHeight() != (2 * maxY + 1)) {
                ...
                // new BufferedImage(...)
                if (buffer != null) {
                    buffer.flush();  // in case Java forgets to clear memory -- bug in OS X
                }
