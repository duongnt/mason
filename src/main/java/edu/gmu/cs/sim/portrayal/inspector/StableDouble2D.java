...
public class StableDouble2D implements StableLocation {
    ...
    void update() {
        Double2D pos = null;
        if (field != null) {
            pos = field.getObjectLocation(object);
        }
        if (pos == null) {
            exists = false;
        }  // purposely don't update x and y so they stay the same
        else {
            x = pos.x;
            y = pos.y;
            exists = true;
    ...
    public void setX(double val) {
        if (field != null) {
            field.setObjectLocation(object, new Double2D(val, getY()));
        }
        x = val;
        exists = true;
    }
    ...
    public void setY(double val) {
        if (field != null) {
            field.setObjectLocation(object, new Double2D(getX(), val));
        }
        y = val;
        exists = true;
    }
