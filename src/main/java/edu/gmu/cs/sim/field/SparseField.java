...
public abstract class SparseField implements java.io.Serializable {
    ...
    protected boolean setObjectLocation(final Object obj, final Object location) {
        if (obj == null) {
            return false;  // RuntimeException("Cannot add null as an object to a SparseField");
        }
        if (location == null) {
            return false;  // RuntimeException("Cannot use null as a location in a SparseField");
        }
        ...
        if (objs == null) {
            ...
            if (canUse != null) {
                canUse.clear();  // clean out and get rid of the cockroaches
            } else {
                canUse = new Bag(INITIAL_BAG_SIZE);
            }
            canUse.add(obj);
