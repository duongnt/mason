...
*/
package edu.gmu.cs.sim.util.distribution;
...
public class Beta extends AbstractContinousDistribution {
    ...
    protected double b00(double a, double b, MersenneTwisterFast randomGenerator) {
        ...
        for (; ; ) {
            if ((U = randomGenerator.nextDouble() * p2) <= p1) {       //  X < t  
                ...
                if ((V = randomGenerator.nextDouble() * fb) <= 1.0 - b_ * X) {
                    break;
                }
                ...
                if ((V = randomGenerator.nextDouble() * fa) <= 1.0 - a_ * (1.0 - X)) {
                    break;
                }
    ...
    public double nextDouble(double alpha, double beta) {
        ...
        if (a == 1.0) {
            if (b != 1.0) {
                return (1.0 - Math.exp(Math.log(randomGenerator.nextDouble()) / b));
            }
            if (b == 1.0) {
