...
public class SimpleColorMap implements ColorMap {
    ...
    public void setLevels(double minLevel, double maxLevel, Color minColor, Color maxColor) {
        if (maxLevel != maxLevel || minLevel != minLevel) {
            throw new RuntimeException("maxLevel or minLevel cannot be NaN");
        }
        if (Double.isInfinite(maxLevel) || Double.isInfinite(minLevel)) {
        ...
        minRed = minColor.getRed();
        minGreen = minColor.getGreen();
        minBlue = minColor.getBlue();
        minAlpha = minColor.getAlpha();
        maxRed = maxColor.getRed();
        maxGreen = maxColor.getGreen();
        maxBlue = maxColor.getBlue();
    ...
    public Color getColor(double level) {
        if (colors != null && level >= 0 && level < colors.length) {
            ...
            if (level != level) {
                level = Double.NEGATIVE_INFINITY;  // NaN handling
            }

            if (level == minLevel) {
                return minColor;  // so we don't divide by zero (maxLevel - minLevel)
            } else if (level == maxLevel) {
                return maxColor;  // so we don't overflow
            }
