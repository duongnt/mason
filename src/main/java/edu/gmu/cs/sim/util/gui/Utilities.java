...
public class Utilities {
    ...
    private static int iFloor(double d) {
        ...
        } else if (d < (Integer.MIN_VALUE + 1))  // handles the fact that negative integer range is one bigger than the positive integer range
        {
            ...
            return i;      // integer negatives
        } else {
            return i - 1;                      // non-integer negatives
        }
    }
    ...
    private static int iCeil(double d) {
        int i = (int) d;
        if (d <= 0) {
            return i;  // negatives and zero -- pretty because doubles pushed to ints don't go lower than the min int range
        } else if (d >= (Integer.MAX_VALUE)) {
            return Integer.MAX_VALUE;
        } else if (i == d) {
            return i;  // integer positives
        } else {
            return (i + 1);            // non-integer positives
        }
    }
    ...
    public static Thread doLater(final long milliseconds, final Runnable doThis) {
        Thread thread = new Thread(new Runnable() {
            public void run() {
                ...
                } catch (InterruptedException e) { /* shouldn't happen -- we could do an invokeAndWait in case tho */ } catch (java.lang.reflect.InvocationTargetException e) { /* shouldn't happen */ }
            }
        });
        thread.start();
        return thread;
