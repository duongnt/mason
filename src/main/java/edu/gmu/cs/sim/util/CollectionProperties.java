...
public class CollectionProperties extends Properties {
    ...
    public CollectionProperties(Object o) {
        ...
        } else if (o instanceof Indexed) {
            buildCollectionProperties((Indexed) o);
        } else if (o instanceof List) {
            buildCollectionProperties((List) o);
        } else if (o instanceof Map) {
            buildCollectionProperties((Map) o);
        } else if (o instanceof Collection) {
            buildCollectionProperties((Collection) o);
        } else if (o.getClass().isArray()) {
            buildCollectionPropertiesForArray(o);
        } else {
    ...
    public Object getValue(int index) {
        ...
        if (indexed != null) {
            return indexed.getValue(index);  // O(1)
        } else  // O(n)
        {
    ...
    public boolean isReadWrite(int index) {
        ...
        if (collection != null) {
            return false;  // collections are not modifiable -- they can't be referenced
        }
    ...
    protected Object _setValue(int index, Object value) {
        ...
        if (indexed != null) {
            indexed.setValue(index, value);  // O(1)
        } else if (map != null) {
            Iterator i = map.entrySet().iterator();
            Object obj = null;
            for (int x = 0; x <= index; x++)  // yes, it's <=
